...
/*
 * test_fidlib_opencl.c — OpenCL Compute FIR engine correctness test
 *
 * Skips gracefully when FIDLIB_OPENCL not compiled in.
 * When an OpenCL device is found, fid_run_new auto-selects the OpenCL path
 * for tap counts >= FIDLIB_OPENCL_THRESHOLD.
 * If no GPU/CL device is available at runtime, fid_run_new falls back to
 * OLA or scalar — the test verifies correctness in all cases.
 */
...
test_impulse(int M)
{
    ...
    if (nonzero_cnt == M)
        printf("PASS  %-52s  count=%d\n", "OpenCL impulse: nonzero count == M", nonzero_cnt);
    else {
        fprintf(stderr, "FAIL  %-52s  got=%d  want=%d\n",
                "OpenCL impulse: nonzero count == M", nonzero_cnt, M);
        g_failed++;
    }

    if (wrong_val == 0)
        printf("PASS  %-52s\n", "OpenCL impulse: all nonzero outputs == 1/M");
    else {
        fprintf(stderr, "FAIL  %-52s  wrong=%d / %d\n",
                "OpenCL impulse: all nonzero outputs == 1/M", wrong_val, nonzero_cnt);
        g_failed++;
    }

    if (first_nz >= 0)
        printf("PASS  %-52s  first_nz_at=%d\n", "OpenCL impulse: saw nonzero output", first_nz);
    else {
        fprintf(stderr, "FAIL  %-52s\n", "OpenCL impulse: never saw nonzero output");
        g_failed++;
    }
...
test_dc(int M)
{
    ...
    chk_near("OpenCL DC steady-state", last, 1.0, TOL);

    fid_run_zapbuf(buf);
    double post_zap = 0.0;
    for (int t = 0; t < 3000; t++) post_zap = fn(buf, 1.0);
    chk_near("OpenCL DC after zapbuf", post_zap, 1.0, TOL);
...
int main(void)
{
#ifndef FIDLIB_OPENCL
    printf("SKIP  FIDLIB_OPENCL not compiled in\n");
    printf("ALL TESTS SKIPPED\n");
    return 0;
#else
    const int M = FIDLIB_OPENCL_THRESHOLD + 1;
    printf("M=%d  threshold=%d\n\n", M, FIDLIB_OPENCL_THRESHOLD);
