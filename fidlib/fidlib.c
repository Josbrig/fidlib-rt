/**
 * @file fidlib.c
 * @brief fidlib — runtime digital filter design and execution engine.
 *
 * This file implements the complete fidlib library: fispec string parsing,
 * all filter design functions (Butterworth, Bessel, Chebyshev, Biquad,
 * resonators), filter analysis (response, phase, delay), and the
 * command-list-based filter execution engine.
 *
 * Internal architecture:
 * @dot
 * digraph FidlibInternals {
 *   rankdir=TB; node [shape=box, fontname=Helvetica, fontsize=9];
 *   spec   [label="fispec string\ne.g. \"LpBu4/400\""];
 *   parse  [label="parse_spec()\nTokenise + resolve frequencies"];
 *   design [label="des_lpbu() / des_hpbu() / …\nFilter family function"];
 *   mkf    [label="fidmkf.h\nbutterworth() / chebyshev() / bessel()\nPole calculation"];
 *   xform  [label="do_lowpass() / do_bandpass() / …\nBilinear transform"];
 *   filt   [label="FidFilter chain\n(heap-allocated)"];
 *   run    [label="fid_run_new()\nCompile to command list"];
 *   cmd    [label="fidrf_cmdlist.h\nRun-phase hot loop"];
 *   spec -> parse -> design;
 *   design -> mkf -> xform -> filt;
 *   filt -> run -> cmd;
 * }
 * @enddot
 *
 * @author  Jim Peters <http://uazu.net/>
 * @copyright LGPL 2.1
 * @ingroup fidlib
 *
 * @note  Fidlib digital filter designer code
...
static void (*error_handler)(const char *err)= 0;
...
static XINLINE void
evaluate(double *rv, const double *coef, int n_coef, double *in) {
...
void
fid_set_error_handler(void (*rout)(const char *)) {
   error_handler= rout;
}
...
double
fid_response_pha(const FidFilter *filt, double freq, double *phase) {
   double top[2], bot[2];
...
double
fid_response(const FidFilter *filt, double freq) {
   double top[2], bot[2];
   double theta= freq * 2 * M_PI;
   double zz[2];
...
int
fid_calc_delay(const FidFilter *filt) {
   FidRun *run;
   FidFunc *dostep;
   void *f1, *f2;
...
stack_filter(int order, int n_head, int n_val, ...) {
   ...
   p= q= rv;
   for (a= 0; a<n_head; a++) {
      p->typ= (short)va_arg(ap, int);
      p->cbm= (short)va_arg(ap, int);
      p->len= va_arg(ap, int);
      for (b= 0; b<p->len; b++) 
   ...
   if ((size_t)len != FFCSIZE(n_head-1, n_val))
      error("Internal error; bad call to stack_filter(); length mismatch (%d,%zu)",
...
des_lpbube(double rate, double f0, double f1, int order, int n_arg, double *arg) {
   ...
   double tmp[MAXPZ];
   int a;
   bessel(order); memcpy(tmp, pol, (size_t)order * sizeof(double));
   butterworth(order); 
...
fid_design(const char *spec, double rate, double freq0, double freq1, int f_adj, char **descp) {
   ...
   if (descp) {
      const char *fmt= filter[sp.fi].txt;
      size_t max= strlen(fmt) + 60U + (size_t)sp.n_arg * 20U;
      ...
      *p++= 0;
      if ((size_t)(p-desc) >= max) error("Internal error: exceeded estimated description buffer");
      *descp= desc;
   }
...
fid_list_filters_buf(char *buf, char *bufend) {
   ...
   for (a= 0; filter[a].fmt; a++) {
      ...
      buf += (cnt= snprintf(buf, (size_t)(bufend-buf), "%s\n    ", tmp));
...
fid_rewrite_spec(const char *spec, double freq0, double freq1, int adj,
   ...
   if (spec1p) {
      ...
      len= strlen(buf);
      rv= (char*)Alloc((size_t)sp.minlen + len + 1U);
      memcpy(rv, spec, (size_t)sp.minlen);
      strcpy(rv+sp.minlen, buf);
      *spec1p= rv;
   }

   if (spec2p) {
      char *rv= (char*)Alloc((size_t)sp.minlen + 1U);
      memcpy(rv, spec, (size_t)sp.minlen);
      *spec2p= rv;
      *freq0p= sp.f0;
      *freq1p= sp.f1;
...
fid_cv_array(double *arr) {
   ...
   for (dp= arr; *dp; ) {
      ...
      ff->cbm= (short)-1;
      ff->len= len;
      memcpy(ff->val, dp, (size_t)len * sizeof(double));
      dp += len;
      ff= FFNEXT(ff);
   }
...
grabWord(char **pp, char *buf, int buflen) {
   ...
   len= q-p;
   if (len >= (ptrdiff_t)buflen) return 0;
...
#define INCBUF { \
 ptrdiff_t _rvp_off= rvp - rv; ptrdiff_t _curr_off= (char*)curr - rv; \
 size_t _sz= (size_t)(rvend - rv) * 2U; \
 tmp= (char*)realloc(rv, _sz); if (!tmp) error("Out of memory"); \
 rv= tmp; rvend= rv + _sz; rvp= rv + _rvp_off; curr= (FidFilter*)(rv + _curr_off); }
   ...
   while (1) {
	 ...
	 buf[0]= 0;
	  ...
	  { ptrdiff_t _rvp_off= rvp - rv;
	    tmp= (char*)realloc(rv, (size_t)_rvp_off + xtra);
	    if (!tmp) error("Out of memory");
	    rv= tmp;
	    curr= (FidFilter*)(rv + _rvp_off); }
	  curr->typ= (short)0; curr->cbm= (short)0; curr->len= 0;
	  *pp= buf[0] ? (p-1) : p;
	  *ffp= (FidFilter*)tmp;
	  return 0;
       case '/':
	 ...
	 while (rvp + (size_t)len + xtra >= rvend) INCBUF;
	 memcpy(rvp, ff, (size_t)len); rvp += len;
	 free(ff);
	 typ= 0;
	 continue;
      }
