...
/**
 * @file fidmkf.h
 * @brief mkfilter-derived S-domain pole/zero synthesis kernel.
 *
 * Implements analogue prototype filter design (Butterworth, Chebyshev I,
 * Bessel) and frequency transformation (LP/HP/BP/BS) followed by a
 * bilinear or matched-Z S→Z conversion.  The resulting Z-domain poles
 * and zeros are packed into a @ref FidFilter chain via z2fidfilter().
 *
 * **Internal header — included directly into `fidlib.c`.  Not a public API.**
 * All declarations here have `static` linkage.
 *
 * @note @threadUnsafe — the global pole/zero workspace (#pol, #zer, etc.)
 *       uses module-level static variables; calling any design function from
 *       two threads simultaneously will corrupt both results.
 *
 * @dot
 * digraph mkfilter_pipeline {
 *   rankdir=LR;
 *   node [shape=box, fontname=Helvetica, fontsize=10];
 *   BU [label="butterworth(order)",    style=filled, fillcolor=lightyellow];
 *   CH [label="chebyshev(order,ripple)",style=filled, fillcolor=lightyellow];
 *   BE [label="bessel(order)",          style=filled, fillcolor=lightyellow];
 *   LP [label="lowpass(freq)"];
 *   HP [label="highpass(freq)"];
 *   BP [label="bandpass(f1,f2)"];
 *   BS [label="bandstop(f1,f2)"];
 *   BU -> LP; BU -> HP; BU -> BP; BU -> BS;
 *   CH -> LP; CH -> HP; CH -> BP; CH -> BS;
 *   BE -> LP;
 *   S2Z [label="s2z_bilinear()\nor s2z_matchedZ()", style=filled, fillcolor=lightblue];
 *   LP -> S2Z; HP -> S2Z; BP -> S2Z; BS -> S2Z;
 *   ZFF [label="z2fidfilter(gain,cbm)\n→ FidFilter chain", style=filled, fillcolor=lightgreen];
 *   S2Z -> ZFF;
 * }
 * @enddot
 *
 * @author  Jim Peters (algorithms derived from mkfilter by Tony Fisher, Univ. of York)
 * @copyright LGPL 2.1
 * @ingroup fidlib
 */
...
/** @brief Maximum poles/zeros supported per prototype filter. */
#define MAXPZ 64

/** @name Global pole/zero workspace
 *  @warning Not thread-safe — one shared workspace for the design phase.
 *  @{
 */
static int    n_pol;              /**< Number of poles currently in workspace. */
static double pol[MAXPZ];         /**< Pole values: real poles store one double; complex
                                       conjugate pairs store real+imag of one representative. */
static char   poltyp[MAXPZ];      /**< Pole type codes: 1=real, 2=first of complex pair, 0=second. */
static int    n_zer;              /**< Number of zeros currently in workspace. */
static double zer[MAXPZ];         /**< Zero values (same encoding as #pol). */
static char   zertyp[MAXPZ];      /**< Zero type codes (same encoding as #poltyp). */
/** @} */
...
/**
 * @brief Place Bessel prototype poles into the workspace.
 * @param order Filter order (1–10).
 * @note Maximum supported order is 10; higher values call error().
 * @ingroup fidlib
 */
static void
bessel(int order) {
   ...
   n_pol= order;
   memcpy(pol, bessel_poles[order-1], (size_t)n_pol * sizeof(double));
...
/**
 * @brief Place Butterworth prototype poles into the workspace.
 *
 * Poles are evenly spaced on the unit circle in the left half-plane:
 * @f$ p_k = e^{j\pi(1 - (n-2k-1)/(2n))} @f$ for @f$ k = 0, 1, \ldots @f$
 *
 * @param order Filter order (up to #MAXPZ).
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Warp Butterworth poles to Chebyshev type I prototype.
 *
 * Must be called immediately after butterworth() with the same order.
 * Scales the unit-circle poles onto an ellipse to equiripple behaviour
 * in the passband.
 *
 * @param order  Filter order (must match previous butterworth() call).
 * @param ripple Passband ripple in dB — **must be negative** (e.g. -0.5 for 0.5 dB).
 *               Positive values trigger error().
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Apply LP frequency transformation to prototype poles.
 * Scales prototype poles by @p freq×2π and places zeros at −∞.
 * @param freq  Cut-off frequency as a fraction of the sampling rate (0–0.5).
 * @ingroup fidlib
 */
static void
lowpass(double freq) {
...
/**
 * @brief Apply HP frequency transformation to prototype poles.
 * Computes the reciprocal of each S-domain pole and scales; zeros go to origin.
 * @param freq  Cut-off frequency as a fraction of the sampling rate.
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Apply BP frequency transformation to prototype poles.
 * Each S-domain pole generates two Z-domain poles; total pole count doubles.
 * @param freq1  Lower band edge as a fraction of the sampling rate.
 * @param freq2  Upper band edge as a fraction of the sampling rate.
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Apply BS (notch) frequency transformation to prototype poles.
 * Like bandpass() but zeros lie on the imaginary axis at ±jω₀.
 * @param freq1  Lower band edge as a fraction of the sampling rate.
 * @param freq2  Upper band edge as a fraction of the sampling rate.
 * @ingroup fidlib
 */
...
/**
 * @brief Convert S-domain poles/zeros to Z-domain via bilinear transform.
 *
 * Applies @f$ z = \dfrac{2 + s}{2 - s} @f$ to every pole and zero.
 * This is the standard pre-warped bilinear mapping used for all
 * Butterworth/Chebyshev/Bessel LP/HP/BP/BS filters.
 * @ingroup fidlib
 */
static void
s2z_bilinear(void) {
...
/**
 * @brief Convert S-domain poles/zeros to Z-domain via matched-Z transform.
 *
 * Applies @f$ z = e^{s} @f$ to every pole and zero.  Used for the
 * resonator filter types (BpRe, BsRe, ApRe) which are designed directly
 * in the Z domain.
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Pack the current Z-domain poles/zeros workspace into a @ref FidFilter chain.
 *
 * The @p gain is prepended as a single-coefficient FIR sub-filter so that
 * the caller can adjust the DC gain of the result easily.  Poles and zeros are
 * paired into 2nd-order IIR/FIR sections (or 1st-order if a lone real pair
 * remains at the end of the list).
 *
 * @param gain  Overall DC gain to embed as the first FIR element.
 * @param cbm   Bitmap of which FIR coefficients are compile-time constants.
 *              Use @c ~0 for LP/HP/BP (all FIR const), @c 0x5 for BS.
 * @return Heap-allocated @ref FidFilter chain (caller must free).
 * @ingroup fidlib
 */
static FidFilter *
z2fidfilter(double gain, int cbm) {
   ...
   rv= (FidFilter*)realloc(rv, (size_t)(((char*)ff)-((char*)rv)));
   if (!rv) error("Out of memory");
   return rv;
}

/**
 * @brief Set up poles/zeros for a band-pass resonator (BpRe).
 * @param freq   Centre frequency as a fraction of the sampling rate.
 * @param qfact  Q-factor.  Pass 0.0 for infinite Q (lossless oscillator).
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Set up poles/zeros for a band-stop resonator (BsRe).
 * @param freq   Notch frequency as a fraction of the sampling rate.
 * @param qfact  Q-factor of the notch.
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Set up poles/zeros for an all-pass resonator (ApRe).
 * @param freq   Phase-shift frequency as a fraction of the sampling rate.
 * @param qfact  Q-factor.
 * @ingroup fidlib
 */
static void
...
/**
 * @brief Set up poles/zeros for a proportional-integral (PI) filter.
 * @param freq  Integrator corner frequency as a fraction of the sampling rate.
 * @ingroup fidlib
 */
static void
